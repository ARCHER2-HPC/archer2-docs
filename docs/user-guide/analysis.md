...
jobs currently running from a single user cannot exceed 32 cores, and the
total memory used by jobs currently running from a single user cannot exceed
128 GB -- any jobs above this limit will remain pending until your previous
jobs are finished.

You do not need to specify both number of cores *and* memory for jobs on the
data analysis nodes. By default, you will get 2 GB of memory per core when
specifying cores only, and 1 core when specifying the memory only.
